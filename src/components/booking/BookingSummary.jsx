    ...
    <div className="glass-panel-luxury rounded-3xl p-8 md:p-10 border border-primary/20 sticky top-28 relative overflow-hidden">
      ...
      <div className="flex items-center gap-4 mb-6 p-4 bg-surface/50 border border-white/10 rounded-2xl overflow-hidden relative">
        {vehicle?.image ? (
          ...
          <div className="w-24 h-20 rounded-xl bg-white/5 flex items-center justify-center">
            <span className="material-symbols-outlined text-white/40 text-3xl">directions_car</span>
          </div>
        )}
        <div>
      ...
      {!ready && missingHint && (
        <div className="text-yellow-400 text-sm mt-2 text-center">{missingHint}</div>
      )}
    </div>
  );
}